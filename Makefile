.PHONY: help env up urls bootstrap down load-data psql logs reset loadtest
SHELL := bash
.DEFAULT_GOAL := help
...
LOADTEST_VARS := SCENARIO VUS DURATION WARMUP SEED COLD_CACHE TILE_FORMAT TILE_LAYERS WMS_LAYERS \
	...
	REPEAT PROFILE BASE COOLDOWN TESTID DISCOVERY_AREAS DISCOVERY_SPREAD LOADTEST_DB_NAME LOADTEST_DB_ROLE
